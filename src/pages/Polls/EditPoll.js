...
import { editPoll, getPollById } from "../../redux/actions/pollActions";
import { message } from "antd";

function EditPoll({ match }) {
  const { selectedPoll } = useSelector((state) => state.pollsReducer);
  let [poll, setPoll] = useState();
  const pollId = match.params.pollId;

  useEffect(() => {
    if (selectedPoll == null || selectedPoll.pollId != pollId) {
      dispatch(getPollById(pollId));
    } else {
      if (selectedPoll.pollId === pollId) {
          ...
          {poll != null && (
            ...
            >
                    ...
                    {fields.map((field) => (
                      ...
                      >
                        <Form.Item
                          {...field}
                          hidden
                          name={[field.name, "pollOptionId"]}
                          fieldKey={[field.fieldKey, "pollOptionId"]}
                        >
              ...
              <div className="text-right">
                <button className="btn1">EDIT POLL</button>
              </div>
            </Form>
          )}
        </Col>
